...
host
\ -code words are not meant to be used in the interpreter
: -code  
   ...
   does> @ ,-t ;
\ code words have a header on the target, can be called in the interpreter
: code  -code header ;
...
\ macros, defined with :m, are like immediate words,
\ they are executed inside colon definitions, mostly
\ to compile loops. Remember that this is a target compiler
\ and there is no real compile time. Like a Forth assembler
\ all the words are executed to build the target, not compiled
:m begin (  - a)  here m;
...
:m for  >r begin m;
:m next  (next) [ 2/ ] , m;
\ 0 is where the C code for docol is in the function array
:m -:  -code  0 , m;
:m :  code  0 , m;
:m ;  exit m;
...
\ it compiles a 32 bit literal.
\ , compiles a 16 bit number into the dictionary,
\ because the AVR program memory is 16 bit word addressed
...
: 1+  1 #, + ;
: 1-  -1 #, + ;
\ type a string from program memory
...
: space  32 #, emit ;
\ type a string from RAM
: type ( a l - )  1- for count emit next drop ;

\ stack contains 32 bit numbers
: * ( n1 n2 - n3)  um* drop ;
...
\ the $a5 at dict will be patched later to point to the headers
